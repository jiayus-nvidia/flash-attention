...
mha_fwd(at::Tensor q,   // (b, s_q, h, d) or (total_q, h, d) if there is cu_seqlens_q
    ...
    TORCH_CHECK(!arbitrary_func_.has_value() ||
                    (!is_causal &&
                     window_size_left < 0 &&
                     window_size_right < 0 &&
                     attention_chunk < 1),
    ...
    if (params.is_arbitrary) {
        ...
        TORCH_CHECK(arbitrary_func.dtype() == torch::kInt32,
                    "arbitrary_func must have dtype int32");
        TORCH_CHECK(arbitrary_func.dim() == 4,
                    "arbitrary_func must be 4D [batch, head_q, func_num, seqlen_q+256]");
        TORCH_CHECK(arbitrary_func.size(2) % 2 == 1,
                    "arbitrary_func func_num (third dimension) must be odd. Got ", arbitrary_func.size(2));
        TORCH_CHECK(arbitrary_func.size(3) >= seqlen_q + 256,
                    ...
                    "Got ", arbitrary_func.size(3), ", expected >= ", seqlen_q + 256);

        params.mask_func_ptr = arbitrary_func.data_ptr<int>();
        // Shape
    ...
    if (use_block_sparsity) {
        ...
        TORCH_CHECK(dprops->major >= 8, "Block sparsity requires Ampere (SM80) or newer GPUs");
        ...
        TORCH_CHECK(block_sparse_mask_cnt.dtype() == torch::kInt32, "block_sparse_mask_cnt must have dtype int32");
        TORCH_CHECK(block_sparse_mask_offset.dtype() == torch::kInt32, "block_sparse_mask_offset must have dtype int32");
        TORCH_CHECK(block_sparse_mask_idx.dtype() == torch::kInt32, "block_sparse_mask_idx must have dtype int32");
        TORCH_CHECK(block_sparse_full_cnt.dtype() == torch::kInt32, "block_sparse_full_cnt must have dtype int32");
        TORCH_CHECK(block_sparse_full_offset.dtype() == torch::kInt32, "block_sparse_full_offset must have dtype int32");
        TORCH_CHECK(block_sparse_full_idx.dtype() == torch::kInt32, "block_sparse_full_idx must have dtype int32");
        ...
        TORCH_CHECK(block_sparse_mask_cnt.dim() == 3,
        "block_sparse_mask_cnt must be 3D [B, H, num_m_blocks]");
        TORCH_CHECK(block_sparse_full_cnt.dim() == 3,
        "block_sparse_full_cnt must be 3D [B, H, num_m_blocks]");
        TORCH_CHECK(block_sparse_mask_offset.dim() == 1,
        "block_sparse_mask_offset must be 1D [B*H*num_m_blocks + 1]");
        TORCH_CHECK(block_sparse_full_offset.dim() == 1,
        "block_sparse_full_offset must be 1D [B*H*num_m_blocks + 1]");
        TORCH_CHECK(block_sparse_mask_idx.dim() == 1,
        "block_sparse_mask_idx must be 1D [total_mask_blocks]");
        TORCH_CHECK(block_sparse_full_idx.dim() == 1,
        "block_sparse_full_idx must be 1D [total_full_blocks]");

        // Validate batch and head dimensions (both can be 1 for broadcasting)
        TORCH_CHECK(block_sparse_mask_cnt.size(0) == batch_size || block_sparse_mask_cnt.size(0) == 1,
        "block_sparse_mask_cnt batch dimension must be 1 or match Q batch size. "
        "Got ", block_sparse_mask_cnt.size(0), ", expected 1 or ", batch_size);
        TORCH_CHECK(block_sparse_mask_cnt.size(1) == num_heads || block_sparse_mask_cnt.size(1) == 1,
        "block_sparse_mask_cnt head dimension must be 1 or match Q num_heads. "
        "Got ", block_sparse_mask_cnt.size(1), ", expected 1 or ", num_heads);
        TORCH_CHECK(block_sparse_full_cnt.size(0) == batch_size || block_sparse_full_cnt.size(0) == 1,
        "block_sparse_full_cnt batch dimension must be 1 or match Q batch size. "
        "Got ", block_sparse_full_cnt.size(0), ", expected 1 or ", batch_size);
        TORCH_CHECK(block_sparse_full_cnt.size(1) == num_heads || block_sparse_full_cnt.size(1) == 1,
        "block_sparse_full_cnt head dimension must be 1 or match Q num_heads. "
        "Got ", block_sparse_full_cnt.size(1), ", expected 1 or ", num_heads);

        // Validate offset size matches cnt size + 1 (CSR format)
        int64_t cnt_numel = block_sparse_mask_cnt.numel();
        TORCH_CHECK(block_sparse_mask_offset.size(0) == cnt_numel + 1,
        "block_sparse_mask_offset size must be block_sparse_mask_cnt.numel() + 1. "
        "Got ", block_sparse_mask_offset.size(0), ", expected ", cnt_numel + 1);
        TORCH_CHECK(block_sparse_full_offset.size(0) == block_sparse_full_cnt.numel() + 1,
        "block_sparse_full_offset size must be block_sparse_full_cnt.numel() + 1. "
        "Got ", block_sparse_full_offset.size(0), ", expected ", block_sparse_full_cnt.numel() + 1);

        // cnt shapes must match between mask and full
        TORCH_CHECK(block_sparse_mask_cnt.sizes() == block_sparse_full_cnt.sizes(),
        ...
        params.block_sparse_mask_cnt = block_sparse_mask_cnt.data_ptr<int>();
        params.block_sparse_mask_offset = block_sparse_mask_offset.data_ptr<int>();
        params.block_sparse_mask_idx = block_sparse_mask_idx.data_ptr<int>();
        params.block_sparse_full_cnt = block_sparse_full_cnt.data_ptr<int>();
        params.block_sparse_full_offset = block_sparse_full_offset.data_ptr<int>();
        params.block_sparse_full_idx = block_sparse_full_idx.data_ptr<int>();
...
std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor> mha_bwd(
    ...
    std::optional<at::Tensor> block_sparse_full_idx_,     // [total_full_blocks]: indices of full blocks
    std::optional<at::Tensor> block_sparse_dq_write_order_,       // [total_mask_blocks]: combined dQ write rank
    std::optional<at::Tensor> block_sparse_dq_write_order_full_,  // [total_full_blocks]: combined dQ write rank
    ...
    TORCH_CHECK(k.get_device() == q.get_device() &&
    ...
    if (params.is_arbitrary) {
        ...
        CHECK_DEVICE(arbitrary_func); CHECK_CONTIGUOUS(arbitrary_func);
        TORCH_CHECK(arbitrary_func.get_device() == q.get_device(),
        ...
        TORCH_CHECK(arbitrary_func.size(3) >= seqlen_q + 256,
                    "arbitrary_func seqlen_q dimension must be greater than or equal to (Q seqlen + 256). "
                    "Got ", arbitrary_func.size(3), ", expected >= ", seqlen_q + 256);
        TORCH_CHECK(arbitrary_func.size(0) == 1 || arbitrary_func.size(0) == batch_size,
                    "arbitrary_func batch dimension must be 1 or match batch size");
        TORCH_CHECK(arbitrary_func.size(1) == 1 || arbitrary_func.size(1) == num_heads,
    ...
    bool const use_block_sparsity = block_sparse_mask_cnt_.has_value() &&
                                    ...
                                    block_sparse_full_offset_.has_value() &&
                                    block_sparse_full_idx_.has_value();
    TORCH_CHECK(!has_any_block_sparsity || use_block_sparsity,
    ...
    if (use_block_sparsity) {
        ...
        // - cnt: [B, H, num_n_blocks] (3D, B, H can be 1 for broadcasting)
        ...
        TORCH_CHECK(!is_arbitrary || block_sparse_mask_cnt.size(2) == (seqlen_k + kBlockN - 1) / kBlockN,
        ...
        auto check_same_device = [&](at::Tensor const &tensor, char const *name) {
            TORCH_CHECK(tensor.get_device() == q.get_device(), name, " must be on the same CUDA device as q");
        };
        ...
        if (deterministic && is_arbitrary) {
            ...
            TORCH_CHECK(block_sparse_dq_write_order.dtype() == torch::kInt32,
                        "block_sparse_dq_write_order must have dtype int32");
            TORCH_CHECK(block_sparse_dq_write_order.dim() == 1,
                        "block_sparse_dq_write_order must be 1D");
            TORCH_CHECK(block_sparse_dq_write_order.numel() == block_sparse_mask_idx.numel(),
            ...
            params.block_sparse_dq_write_order = block_sparse_dq_write_order.data_ptr<int>();

            auto validate_semantics = [&](at::Tensor const* full_rank) {
                if (!unsafe_skip_block_sparse_semantic_validation) {
                    auto validation_stream = at::cuda::getCurrentCUDAStream().stream();
            ...
            if (block_sparse_dq_write_order_full_.has_value()) {
                ...
                TORCH_CHECK(block_sparse_dq_write_order_full.dtype() == torch::kInt32,
                            "block_sparse_dq_write_order_full must have dtype int32");
                TORCH_CHECK(block_sparse_dq_write_order_full.dim() == 1,
                            "block_sparse_dq_write_order_full must be 1D");
                TORCH_CHECK(block_sparse_dq_write_order_full.numel() == block_sparse_full_idx.numel(),
                ...
                params.block_sparse_dq_write_order_full = block_sparse_dq_write_order_full.data_ptr<int>();
                validate_semantics(&block_sparse_dq_write_order_full);
            } else {
                TORCH_CHECK(block_sparse_full_idx.numel() == 0,
