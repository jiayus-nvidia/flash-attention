...
def _native_sm90_bwd_smem_bytes(
    ...
    mma_dkv_is_rs = (
        config.AtomLayoutMSdP == 1
        and config.AtomLayoutNdKV == config.num_wg
        and config.SdP_swapAB
        and not config.dKV_swapAB
    )
...
def sm90_native_bwd_can_implement(
    ...
    """Match native SM90 public causal-forward/backward coverage."""
    ...
    config = _tile_size_bwd_sm90(head_dim, head_dim_v, True, False)
...
def resolve_sm90_bwd_consumer_config(
    *,
    ...
    # Match the native causal consumer configuration. Arbitrary
    # backward changes only K2Q traversal and payload application; it does not
    # select dimension-specific tiles or pipeline stages.
    cfg = _tile_size_bwd_sm90(head_dim, head_dim_v, True, False)
    ...
    payload_values_per_thread, remainder = divmod(
        cfg.m_block_size * cfg.n_block_size, num_mma_threads
    )
    if remainder:
