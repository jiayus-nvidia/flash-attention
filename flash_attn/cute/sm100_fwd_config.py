    ...
    *,
    ...
    if arch // 10 not in (10, 11):
        raise NotImplementedError(
            "SM100 arbitrary forward config supports SM100/SM110 families only"
        )
    ...
    if not (is_standard_shape or is_d192_v128):
        raise NotImplementedError(
            "generic SM100 arbitrary forward supports D/Dv <= 128 or (192, 128); "
    ...
    # Match the generic dispatch first: SM100 derives q_stage from the Q/KV
    # head ratio even when PackGQA is disabled, while SM110 remains single-stage.
    effective_q = max_seqlen_q * qhead_per_kvhead
    q_stage = 2 if canonical_blackwell_arch_family(arch) == "sm100" and effective_q > tile_m else 1
    cta_group_size = 1
    cluster_axis = "m"
...
def make_sm100_fwd_tiled_mma_qk(
    ...
    """Build the QK MMA layout shared by planner and the 1CTA consumer."""
