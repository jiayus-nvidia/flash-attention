...
def _validate_common_compact_plan(
    ...
    *,
    ...
    if tensors.block_size != block_size:
        raise ValueError(
            f"{context} block_size={tensors.block_size} does not match "
    ...
    if expected_hmask is not None and hmask != expected_hmask:
        raise ValueError(
            f"{context} Hmask={hmask} does not match forward plan Hmask={expected_hmask}"
        )
...
def normalize_arbitrary_block_sparse_config(
    ...
    *,
    ...
    if is_sm100_forward:
        ...
        num_scheduled_heads = (
            num_q_heads // tensors.plan_signature.qhead_per_kvhead
            if pack_gqa
            else num_q_heads
        )
        ...
        if tensors.fwd_work_desc.shape[0] != expected_tasks:
            raise ValueError(
                "fwd_work_desc task count does not match compact Q rows and scheduled heads"
            )
        if not is_fake_mode() and tensors.fwd_work_desc.data_ptr() % 16 != 0:
            raise ValueError("fwd_work_desc must be 16-byte aligned")
        needs_sequence_desc = (
            is_varlen
            or tensors.plan_signature.kernel_family.startswith("sm100_hd256")
        )
