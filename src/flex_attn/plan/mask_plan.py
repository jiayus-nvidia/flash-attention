...
if TYPE_CHECKING:
    from flex_attn.plan.validation import PlanGeometry
    ...
    def debug_snapshot(self) -> dict[str, object]:
        ...
        def snapshot_view(view: object | None) -> dict[str, torch.Tensor | None] | None:
            ...
            return {
                name: (
                    value.detach().clone()
                    if isinstance((value := getattr(view, name, None)), torch.Tensor)
                    else None
                )
                for name in names
            }
        ...
        return {
            ...
            "cu_seqlens_q": (
                self._cu_seqlens_q.detach().clone()
                if self._cu_seqlens_q is not None
                else None
            ),
            "cu_seqlens_k": (
                self._cu_seqlens_k.detach().clone()
                if self._cu_seqlens_k is not None
                else None
            ),
        }

    def _validate_runtime(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, *, mode: str) -> None:
        metadata = self._metadata
        if mode != metadata.mode:
            raise ValueError(f"{mode} attention cannot consume a {metadata.mode} MaskPlan")
