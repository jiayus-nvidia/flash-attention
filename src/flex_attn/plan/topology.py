...
from flex_attn.kernels.sm100.fwd.forward_config import _ResolvedSm100FwdConsumerConfig
...
def _consumer_plan_signature(config) -> ArbitraryPlanSignature:
    ...
    if isinstance(config, _ResolvedSm90BwdConsumerConfig):
        return ArbitraryPlanSignature(
            ...
            mma_atom_layout_id=(
                f"sm90_wgmma_f32_ss_sdp_m{config.tile_m}n{config.tile_n}"
                f"_atom_m{config.atom_layout_m_sdp}_wg{config.num_wg}_major_kk"
            ),
