...
from flex_attn.kernels.common import copy_utils
...
class _ResolvedSm100BwdConsumerConfig:
    ...
    def plan_signature(self) -> ArbitraryPlanSignature:
        ...
        return ArbitraryPlanSignature(
            ...
            mma_atom_layout_id=(
                f"tcgen05_f32_ss_kq_cta{topology.cta_group_size}"
                f"_m{self.sparse_tile_n}n{self.tile_m}_major_kk"
            ),
            swap_ab=True,
    ...
    *,
    ...
    if not (is_standard_shape or is_d192_v128):
        raise NotImplementedError(
            "generic SM100 arbitrary backward supports D/Dv <= 128 or (192, 128); "
    ...
    if subtile_factor != 1:
        raise NotImplementedError(
            "SM100/SM103 arbitrary backward currently requires subtile_factor=1"
        )
    ...
    if remainder:
        raise AssertionError(
            "SM100 backward score values must partition evenly across compute threads"
        )
    ...
    payload_layout_id = (
        ...
        + (
            f"_k{sparse_tile_n}_ctarank{cta_group_size}_axis{cluster_axis}_v2"
            if requires_cooperative_ctas
            else "_v1"
        )
    )
