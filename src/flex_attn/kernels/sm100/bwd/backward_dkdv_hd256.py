...
import flex_attn.kernels.common.copy_utils as fa_copy_utils
...
def _hd256_bwd_sparse_head_state(
    ...
    curr_q_cnt, curr_q_idx, curr_full_cnt, curr_full_idx, partial_base, full_base, _ = (
        get_block_sparse_iteration_info_bwd(
    ...
    full_count = Int32(0)
    if cutlass.const_expr(curr_full_idx is not None):
...
def _hd256_bwd_sparse_head_loop_count(
    ...
    curr_q_cnt, curr_q_idx, curr_full_cnt, curr_full_idx, _, _, _ = (
        get_block_sparse_iteration_info_bwd(
            block_sparse_tensors,
            batch_idx,
            head_idx,
...
def _hd256_bwd_sparse_group_first_work_item(
    ...
    partial_base, full_base, partial_count, full_count, head_count = (
        _hd256_bwd_sparse_head_state(
            block_sparse_tensors,
            batch_idx,
            head_idx,
            n_block,
...
class BlackwellFusedMultiHeadAttentionBackwardDKDVKernel:
    ...
    def __init__(
        ...
        # dSK defines the shared-memory layout reused by the P transpose path.
        self.dSK_mma_tiler = (
            cta_tiler[0] * 2,
            cta_tiler[2],
            cta_tiler[1],
    ...
    def __call__(
        ...
        # Define the dS layout used by the P transpose path.
        ...
        # This layout backs the transposed P staging buffer.
    ...
    def reg_to_smem_mma64x64(
        ...
        # Map the register fragment to the SMEM transpose layout.
        ...
        # The following layout is specialized for the 64 x 64 tile.
