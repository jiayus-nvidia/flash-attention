...
from flex_attn.kernels.common.block_info import BlockInfo
...
from flex_attn.runtime.dsl_utils import bulk_copy, assume_tensor_aligned, struct_scalar_ptr
from flex_attn.kernels.sm100.bwd.named_barrier import NamedBarrierBwdSm100
...
class FlexAttentionBackwardSm100:
    ...
    def __call__(
        ...
        self.dKV_postprocess = (
            self.qhead_per_kvhead > 1
            or self.check_hdim_oob
            or self.check_hdim_v_oob
        )
        self.use_tma_store = self.dKV_postprocess or not (
        ...
        if const_expr(self.use_2cta_instrs):
            assert (
                ...
                "SM100/SM103 D=128, Dv=128 or D=192, Dv=128."
            )
    ...
    def kernel(
        ...
        umma_pair_base_override = (
            0 if const_expr(self.use_2cta_instrs) else None
        )
    ...
    def relay(
        ...
        while work_tile.is_valid_tile:
            ...
            process_tile = (
                const_expr(not self.is_varlen_q) or m_block_min < m_block_max
            )
    ...
    def split_wg(
        ...
        if const_expr(reduced_shape[1] > 1):
            ...
            if const_expr(rank == 3 and reduced_shape[2] >= num_wg):
                ...
                assert reduced_shape[0] % num_wg == 0, (
                    "The leading mode must divide the compute warp groups"
                )
                t = cute.logical_divide(
                    t,
                        ...
                        for i in cutlass.range_constexpr(tdKgdK.shape[1]):
                            ...
                            if row_idx < seqlen.seqlen_k - cluster_tile_n * n_block_for_tile:
                                for j in cutlass.range_constexpr(tdKgdK.shape[2]):
                                    if (
                                        not const_expr(self.check_hdim_oob)
                                        or tdKcdK[0, i, j][1] < self.head_dim
                                    ):
                                        cute.copy(
                        ...
                        for i in cutlass.range_constexpr(tdVgdV.shape[1]):
                            ...
                            if row_idx < seqlen.seqlen_k - cluster_tile_n * n_block_for_tile:
                                for j in cutlass.range_constexpr(tdVgdV.shape[2]):
                                    if (
                                        not const_expr(self.check_hdim_v_oob)
                                        or tdVcdV[0, i, j][1] < self.head_dim_v
                                    ):
                                        cute.copy(
