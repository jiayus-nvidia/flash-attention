...
import flex_attn.kernels.common.pipeline as pipeline_custom
...
from flex_attn.kernels.common.softmax import SoftmaxSm100
...
class _FlexAttentionForwardSm100Base:
    """Shared launch, warp-role, softmax, and epilogue implementation."""
    ...
    def __init__(
        ...
        _enable_qstage1_n_direction: bool = False,
    ):
        ...
        self.n_direction_qstage1 = _enable_qstage1_n_direction
        if self.q_stage == 1 and not self.n_direction_qstage1:
            raise NotImplementedError(
                "generic qstage1 forward must use the N-direction pipeline"
            )
        self.score_stage = 2 if self.n_direction_qstage1 else self.q_stage
        self.use_2cta_instrs = use_2cta_instrs
        self.output_stage = (
            2
            if self.n_direction_qstage1 and not self.use_2cta_instrs
            else self.q_stage
        )
        ...
        assert self.arch.is_family_of(Arch.sm_100f), "Only SM100/SM103 are supported"
        ...
        if not is_persistent:
            raise NotImplementedError("SM100/SM103 forward requires the CLC persistent scheduler")
        if persistent_block_major:
            raise NotImplementedError(
                "SM100/SM103 forward block-major static scheduling is unsupported; "
                "the mask plan must provide the CLC task order"
            )
        if not use_clc_scheduler:
            raise NotImplementedError("SM100/SM103 forward cannot disable the CLC persistent scheduler")
        self.is_persistent = True
        self.persistent_block_major = False
        if self.use_2cta_instrs and not self.n_direction_qstage1:
            raise NotImplementedError(
                "generic SM100 arbitrary forward only exposes 2CTA through the "
                "qstage1 candidate"
            )
        if self.n_direction_qstage1:
            if self.q_stage != 1:
                raise ValueError("qstage1 N-direction candidate requires q_stage=1")
            if not sm100_generic_fwd_shape_is_supported(head_dim, head_dim_v):
                raise ValueError(
                    "generic qstage1 candidate supports D/Dv in [8, 128] "
                    "or (192, 128)"
                )
        ...
        self.use_tma_O = (
            not (
                self.pack_gqa
                and self.m_block_size % self.qhead_per_kvhead != 0
            )
            and not is_varlen_q
        )
        ...
        # SM103 uses native exp2; SM100 keeps the existing emulation policy.
        ...
        self.TileScheduler = PlanClcPersistentTileSchedulerSm100

        flex_log(1, f"TileScheduler={self.TileScheduler.__name__}, USE_2CTA={self.use_2cta_instrs}")
        ...
        self.tmem_o_offset = [
            ...
            for i in range(self.score_stage)
        ]  # e.g., 256, 384
    ...
    def _setup_attributes(self):
        ...
        # Match FA4's cap for small head dimensions. The raw SMEM formula does
        # not account for every per-stage object and can otherwise over-stage.
        kv_stage = min(
        ...
        if self.n_direction_qstage1:
            # Leave room for barriers, scale vectors, CLC state, and aligned sO.
            max_qstage1_kv_stage = 7 if self.use_2cta_instrs else 4
            self.kv_stage = min(kv_stage, max_qstage1_kv_stage)
            if (
                not self.use_2cta_instrs
                and self.head_dim_padded == 192
                and self.head_dim_v_padded == 128
            ):
                # The qstage1 N-direction producer starts with K0/K1/V0.
                # A three-stage uneven [large, small, large] ring would let V0
                # overlap the still-live K1 stage, so use two regular KV stages.
                self.kv_stage = 2
        self.s_stage = 2
    ...
    def __call__(
        ...
        # (s_k, d, h_k, b_k) or (total_k, d, h_k)
        ...
        sO_layout = sm100_utils_basic.make_smem_layout_epi(
            self.o_dtype, self.o_layout, self.epi_tile, self.output_stage
        )
        ...
        class SharedStorage:
            ...
            mbar_P_full_lastsplit: cute.struct.MemRange[Int64, self.score_stage * 2]
            mbar_O_full: cute.struct.MemRange[Int64, self.score_stage * 2]
            mbar_softmax_stats: cute.struct.MemRange[Int64, self.score_stage * 2]
            # mbar_softmax_stats: cute.struct.MemRange[Int64, self.q_stage * 4 * 2]
            mbar_O_epi: cute.struct.MemRange[Int64, self.output_stage * 2]
            ...
            sK: cute.struct.Align[
                # K and V alias this buffer, so size it for both physical views.
    ...
    def kernel(
        ...
        mK: cute.Tensor,  # (s_k, d, h_k, b_k) or (total_k, d, h_k)
        ...
        tOtO = thr_mma_pv.make_fragment_C(cute.append(pv_acc_shape, self.score_stage))
        ...
        tOrP = cute.make_tensor(
            ...
            cute.append(
                tOrP.layout,
                cute.make_layout((self.score_stage,), stride=(tP_stage_stride,)),
            )
        )

        SeqlenInfoCls = partial(
            SeqlenInfoQK.create_from_descriptor,
            seqlen_q_static=mQ.shape[0] if const_expr(not self.pack_gqa) else mQ.shape[0][1],
            seqlen_k_static=mK.shape[0],
            mSequenceDesc=blocksparse_tensors.sequence_desc,
            mCuBlockIdxOffsets=(
        ...
        # ///////////////////////////////////////////////////////////////////////////////
        if warp_idx <= (
            self.softmax1_warp_ids[-1]
            if const_expr(self.n_direction_qstage1 or self.q_stage == 2)
            else self.softmax0_warp_ids[-1]
        ):
    ...
    def softmax_loop(
        ...
        thr_tmem_load = make_sm100_fwd_tmem_load(tSAcc, tidx, use_ldred=False)
        thr_tmem_load_full = make_sm100_fwd_tmem_load(
            tSAcc, tidx, use_ldred=self.use_ldred_rowmax
        )
        tStS_t2r = thr_tmem_load.partition_S(tSAcc)  # (((32,32),1),1,4)
        tStS_t2r_full = thr_tmem_load_full.partition_S(tSAcc)
            ...
            if not empty_tile:
                ...
                if const_expr(mLSE is not None or self.n_direction_qstage1):
                    sScale[
                        ...
                        + self.score_stage * self.m_block_size
                    ] = softmax.row_max[0]
    ...
    def softmax_step(
        ...
        thr_tmem_load_full: cute.CopyAtom,
        thr_tmem_store: cute.CopyAtom,
        thr_tmem_store_scale: cute.CopyAtom,
        tStS_t2r: cute.Tensor,
        tStS_t2r_full: cute.Tensor,
        ...
        if const_expr(self.use_ldred_rowmax and mask_fn is None):
            ...
            tSrS_red = cute.make_rmem_tensor(
                ((1, 1), *tSrS_t2r.shape[1:]), self.qk_acc_dtype
            )
            cute.copy(
                thr_tmem_load_full,
                tStS_t2r_full,
        ...
        if const_expr(self.use_ldred_rowmax and mask_fn is None):
            row_max, acc_scale = softmax.update_row_max_precomputed(
                hw_row_max, is_first
            )
        else:
    ...
    def epilogue_s2g(
        ...
        epi_consumer_state = pipeline.make_pipeline_state(
            pipeline.PipelineUserType.Consumer, self.output_stage
        )
        ...
        while work_tile.is_valid_tile:
            ...
            if has_work:
                ...
                if const_expr(self.n_direction_qstage1):
                    pipeline_o_epi.consumer_wait(epi_consumer_state)
                    output_stage = epi_consumer_state.index
                    assert self.use_tma_O
                    store_O, _, _ = copy_utils.tma_get_copy_fn(
                        tma_atom_O, 0, cute.make_layout(1), sO, gO
                    )
                    store_O(src_idx=output_stage, dst_idx=0)
                    cute.arch.cp_async_bulk_commit_group()
                    cute.arch.cp_async_bulk_wait_group(0, read=True)
                    pipeline_o_epi.consumer_release(epi_consumer_state)
                    epi_consumer_state.advance()

                if const_expr(not self.n_direction_qstage1 and self.use_tma_O):
                    store_O, _, _ = copy_utils.tma_get_copy_fn(
                        tma_atom_O, 0, cute.make_layout(1), sO, gO
                    )
                ...
                elif const_expr(not self.n_direction_qstage1):
                    epilogue_threads = (
                        cute.arch.WARP_SIZE * len(self.epilogue_warp_ids)
                    )
                    tidx = cute.arch.thread_idx()[0] % epilogue_threads
                    for stage in cutlass.range_constexpr(self.q_stage):
                        # wait from corr, issue tma store on smem
                        # 1. wait for O0 / O1 final
                        pipeline_o_epi.consumer_wait_w_index_phase(stage, epi_consumer_phase)
                        ...
                        virtual_warps = (
                            ...
                            if const_expr(
                                self.n_direction_qstage1 and self.pack_gqa
                            )
                            else 1
                        )
                        for virtual_warp in cutlass.range_constexpr(
                            virtual_warps
                        ):
                            self._store_O_to_gmem(
                                sO[None, None, stage],
                                ...
                                mO_cur,
                                gmem_tiled_copy_O,
                                tidx + virtual_warp * epilogue_threads,
                                seqlen.seqlen_q,
                                m_tile_idx,
                            )
                        pipeline_o_epi.consumer_release_w_index(stage)

                if const_expr(not self.n_direction_qstage1):
