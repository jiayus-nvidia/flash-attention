...
class FlexAttentionForwardQStage1Sm100(_FlexAttentionForwardSm100Base):
    ...
    def __init__(
        ...
        if m_block_size != 128 or n_block_size != 128:
            raise NotImplementedError(
                "generic qstage1 forward requires M128xN128 per CTA"
            )
        if q_stage != 1:
            raise ValueError("generic qstage1 forward requires q_stage=1")
        if use_2cta_instrs and overlap_pv_with_k_wait:
            raise ValueError(
                "overlap_pv_with_k_wait is only supported by qstage1 1CTA"
            )
        ...
        if not self.use_tma_Q:
            raise NotImplementedError(
                "generic qstage1 forward requires a PackGQA ratio divisible into M128"
            )
    ...
    def correction_loop(
        ...
        corr_epi_producer_state = pipeline.make_pipeline_state(
            pipeline.PipelineUserType.Producer, self.output_stage
        )
        ...
        while work_tile.is_valid_tile:
            ...
            for stage in cutlass.range_constexpr(self.score_stage):
                stage_count = (
                    stage_count_s0 if const_expr(stage == 0) else stage_count_s1
                )
            ...
            for stream_ordinal in cutlass.range(
                Int32(1), max_stream_count, unroll=1
            ):
            ...
            row_max_safe = (
                row_max_combined
                if row_max_combined != -Float32.inf
                else Float32(0.0)
            )
            scale0 = (
                cute.math.exp2(
            ...
            combined_invalid = (
                row_sum_combined <= Float32(0.0)
                or row_sum_combined != row_sum_combined
            )
            inv_row_sum = cute.arch.rcp_approx(
                row_sum_combined if not combined_invalid else Float32(1.0)
            )
            ...
            if const_expr(mMaxLogit is not None):
                ...
                update_max_logit(
                    ...
                    row < seqlen_q
                    and (
                        not combined_invalid
                        or softmax_scale == Float32(0.0)
                    ),
            ...
            if const_expr(mLSE is not None):
                ...
                lse = (
                    (
                        ...
                        + cute.math.log2(row_sum_combined, fastmath=True)
                    )
                    * math.log(2.0)
            ...
            if const_expr(
                pipeline_load_epi is not None and self.use_correction_warps_for_epi
            ):
        ...
        for i in cutlass.range(
            ...
            if scale0 != Float32(0.0):
                for j in cutlass.range(
                    0, cute.size(tOrO0), 2, unroll_full=True
                ):
            ...
            if scale1 != Float32(0.0):
                for j in cutlass.range(
                    0, cute.size(tOrO1), 2, unroll_full=True
                ):
    ...
    def mma(
        ...
        sm100_utils.declare_ptx_idesc(
            qk_mma_op, var_name="flex_fwd_qstage1_qk_mma_idesc"
        )
        sm100_utils.declare_ptx_idesc(
            pv_mma_op, var_name="flex_fwd_qstage1_pv_mma_idesc"
        )
        ...
        while work_tile.is_valid_tile:
            ...
            if block_iter_count > Int32(0) and is_leader_cta:
                ...
                for epilogue_ordinal in cutlass.range(
                    epilogue_begin, block_iter_count, unroll=1
                ):
                    pipeline_kv.consumer_wait(mma_kv_consumer_state)
                    v_index, v_phase = (
                        mma_kv_consumer_state.index,
