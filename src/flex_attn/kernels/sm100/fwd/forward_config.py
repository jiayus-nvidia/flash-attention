...
def _validate_sm100_generic_fwd_shape(
    ...
    if not sm100_generic_fwd_shape_is_supported(head_dim, head_dim_v):
        raise NotImplementedError(
            "generic SM100 arbitrary forward supports D/Dv in [8, 128] "
            "or (192, 128); "
    ...
    if head_dim % alignment != 0 or head_dim_v % alignment != 0:
        raise ValueError(
            f"head_dim and head_dim_v must be divisible by {alignment} for {dtype}"
        )
...
class _ResolvedSm100FwdConsumerConfig:
    ...
    def plan_signature(self) -> ArbitraryPlanSignature:
        ...
        kernel_family = (
            "sm100_qstage1_1cta_fwd"
            if is_qstage1_1cta
            else (
                "sm100_qstage1_2cta_fwd"
                if is_qstage1_2cta
                else "sm100_generic_fwd"
            )
        )
        return ArbitraryPlanSignature(
            ...
            mma_atom_layout_id=(
                f"tcgen05_f32_ss_qk_cta{self.cta_group_size}"
                f"_m{self.tile_m * self.cta_group_size}"
                f"n{self.tile_n}_major_kk"
            ),
            swap_ab=False,
...
def resolve_sm100_fwd_consumer_config(
    *,
    ...
    if arch not in (100, 103):
        raise NotImplementedError(
            "SM100 arbitrary forward config supports SM100/SM103 only"
        )
    ...
    payload_values_per_thread, remainder = divmod(
        tile_m * tile_n, _SM100_SOFTMAX_THREADS_PER_SUBTILE
    )
    if remainder:
    ...
    payload_layout_id = (
        ...
        f"_v{payload_values_per_thread}_w{payload_padded_words}_v1"
    )
...
def resolve_sm100_fwd_qstage1_2cta_consumer_config(
    *,
    ...
    if arch not in (100, 103):
        raise NotImplementedError(
            "generic qstage1 2CTA arbitrary forward supports SM100/SM103 only"
        )
    if dtype not in (torch.float16, torch.bfloat16):
        raise NotImplementedError(
            "generic qstage1 2CTA arbitrary forward supports FP16 and BF16 only"
        )
    if type(is_varlen) is not bool:
        raise TypeError("is_varlen must be a bool")
    _validate_sm100_generic_fwd_shape(dtype, head_dim, head_dim_v)
...
def resolve_sm100_fwd_qstage1_1cta_consumer_config(
    *,
    ...
    if arch not in (100, 103):
        raise NotImplementedError(
            "generic qstage1 1CTA arbitrary forward supports SM100/SM103 only"
        )
    if dtype not in (torch.float16, torch.bfloat16):
        raise NotImplementedError(
            "generic qstage1 1CTA arbitrary forward supports FP16 and BF16 only"
        )
    if type(is_varlen) is not bool:
        raise TypeError("is_varlen must be a bool")
    _validate_sm100_generic_fwd_shape(dtype, head_dim, head_dim_v)
...
def make_sm100_fwd_tiled_mma_qk(
    ...
    cta_group = (
        ...
        if cutlass.const_expr(cta_group_size == 2)
