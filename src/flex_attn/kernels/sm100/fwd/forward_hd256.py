...
class BlackwellFusedMultiHeadAttentionForward:
    def __init__(
        ...
        self.tmem_alloc_barrier = pipeline.NamedBarrier(
            ...
            * (1 + len(self.softmax_warp_ids) + len(self.correction_warp_ids)),
        )
    ...
    def __call__(
        ...
        if cutlass.const_expr(blocksparse_tensors is not None):
            ...
            if cutlass.const_expr(mCuSeqlensQ is not None):
                assert blocksparse_tensors.sequence_desc is not None, (
                    "SM100 hd256 arbitrary varlen forward requires sequence descriptors"
                )
        ...
        sequence_desc_qk = (
            blocksparse_tensors.sequence_desc
            if cutlass.const_expr(mCuSeqlensQ is not None)
            else None
        )
    ...
    def kernel(
        ...
        if warp_idx == self.load_warp_id:
            ...
            while work_tile.is_valid_tile:
                curr_block_coord = _hd256_work_coord(
                    work_tile, mma_tile_coord_v, Int32(self.cta_group_size)
                )
                mma_block_coord = (
        ...
        if warp_idx == self.v_load_warp_id:
            ...
            while v_work_tile.is_valid_tile:
                v_block_coord = _hd256_work_coord(
                    v_work_tile, mma_tile_coord_v, Int32(self.cta_group_size)
                )
                v_mma_block_coord = (
                ...
                if cutlass.const_expr(mSequenceDescQ is not None):
                    ...
                    v_continue_cond = (
                        v_mma_block_coord[0] * self.qk_mma_tiler[0] >= v_seqlen_q
                    )
                    if cutlass.const_expr(mSequenceDescK is not None):
    ...
    def correction_epilog_tma(
        ...
        tidx, _, _ = cute.arch.thread_idx()
        epilogue_threads: cutlass.Constexpr[int] = self.threads_per_warp * len(
            self.correction_warp_ids
        )
    ...
    def store_sum_max(
        ...
        if cutlass.const_expr(mLSE is not None):
            ...
            if cute.elem_less(q_idx, seqlen_q):
                global_q_idx = (
                    q_idx + cuseqlen_q if cutlass.const_expr(mSequenceDescQ is not None) else q_idx
                )
