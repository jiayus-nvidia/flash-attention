...
import flex_attn.kernels.common.device_utils as utils
from flex_attn.kernels.common.fast_math import clz
...
class TileSchedulerArguments(ParamsBase):
    ...
    persistent_block_major: cutlass.Constexpr[bool] = False
...
class SingleTileMaxVarlenScheduler:
    ...
    def get_current_work(self, *, loc=None, ip=None) -> WorkTileInfo:
        ...
        if batch_idx < self.params.num_batch:
            if const_expr(self.params.mSeqUsed is not None):
                seqlen = self.params.mSeqUsed[batch_idx]
            else:
                assert self.params.mCuSeqlens is not None
                seqlen = self.params.mCuSeqlens[batch_idx + 1] - self.params.mCuSeqlens[batch_idx]
        valid = (
            self._is_first_block
            and batch_idx < self.params.num_batch
            and block_idx * self.params.tile_size < seqlen
        )
...
class StaticPersistentTileScheduler:
    ...
    def get_current_work(self, *, loc=None, ip=None) -> WorkTileInfo:
        if const_expr(self.params.block_major):
            block_idx, hn_idx = divmod(self._tile_idx, self.params.tile_divmod)
        else:
            hn_idx, block_idx = divmod(self._tile_idx, self.params.tile_divmod)
...
class SingleTileLPTScheduler:
    ...
    def get_grid_shape(
        ...
        *,
        ...
        if const_expr(params.use_cluster_idx):
            return (params.total_blocks * params.cluster_shape_m, Int32(1), Int32(1))
    ...
    def clc_work_to_coords(self, work) -> WorkTileInfo:
        ...
        CLC returns raw grid coordinates without L2 swizzling.
        """
...
class SingleTileLPTBwdScheduler:
    ...
    def get_current_work(self, *, loc=None, ip=None) -> cutlass.utils.WorkTileInfo:
        ...
        if const_expr(params.mSeqUsed is not None):
            seqlen = params.mSeqUsed[batch_idx]
        elif const_expr(params.mCuSeqlens is not None):
...
class SingleTileVarlenScheduler:
    ...
    def create(
        ...
        if const_expr(params.use_cluster_idx):
            # Cooperative CTAs must decode the same logical work tile.  The
            # cluster rank is applied later by the kernel's consumer mapping.
            tile_idx = cute.arch.cluster_idx()[0]
    ...
    def get_grid_shape(
        ...
        *,
        ...
        if const_expr(params.is_persistent):
            ...
            sm_count = hardware_info.get_device_multiprocessor_count()
            total_blocks = cutlass.min(sm_count, total_blocks)
    ...
    def _varlen_coord_map(self) -> WorkTileInfo:
        ...
        if cutlass.const_expr(params.use_cluster_idx):
