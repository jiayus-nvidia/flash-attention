        ...
        for m in cutlass.range_constexpr(cute.size(tOrO.shape[1])):
            ...
            if (
                ...
                for k in cutlass.range_constexpr(cute.size(tOrO.shape[2])):
                    ...
                    # Supported head dimensions are vector aligned, so guard the
                    # whole copy atom instead of building a scalar predicate for
                    # its vector-shaped source fragment.
                    if (
                        not cutlass.const_expr(self.check_hdim_oob)
                        or tOcO[0, 0, k][1] < mO.shape[1]
                    ):
                        cute.copy(
                            gmem_thr_copy,
                            tOrO[None, m, k],
