...
from flex_attn.kernels.common.pack_gqa import PackGQA
from flex_attn.kernels.sm90.named_barrier import NamedBarrierFwd
from flex_attn.plan.kernels import BlockSparseTensors
from flex_attn.kernels.common.tile_scheduler import SingleTileScheduler, SingleTileVarlenScheduler, TileSchedulerArguments
        ...
        if const_expr(mMaxLogit is not None):
            if taccOcO[0][1] == 0:
                for m in cutlass.range(cute.size(row_max), unroll_full=True):
                    ...
                    row_is_valid = row < seqlen.seqlen_q * (
