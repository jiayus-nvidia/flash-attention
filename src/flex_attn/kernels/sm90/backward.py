...
from flex_attn.kernels.sm90.backward_config import make_sm90_bwd_tiled_mma_sdp
...
class FlexAttentionBackwardSm90:
    arch = 90
    ...
    def __call__(
        ...
        # Fixed and varlen inputs use different tensor ranks.
    ...
    def load(
        ...
        if warp_idx_in_wg == 0:
            ...
            while work_tile.is_valid_tile:
                ...
                # Arbitrary K2Q loads consume LSE/dPsum written by preprocess,
                # and dQ stores require preprocess to finish zeroing dQaccum.
        ...
        while work_tile.is_valid_tile:
            ...
            consumer_state_Q, consumer_state_dO, process_tile = (
                consume_block_sparse_mma_bwd_sm90(
                    ...
                    pipeline_Q=pipeline_Q,
                    n_blocks_per_sample=cute.ceil_div(seqlen.seqlen_k, self.tile_n),
                    payload_words=(self.tile_m * self.tile_n // self.num_mma_threads + 31) // 32,
                    subtile_factor=self.subtile_factor,
                    m_block_max=m_block_max,
                )
            )
