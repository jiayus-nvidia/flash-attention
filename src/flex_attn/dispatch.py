...
from flex_attn.plan.kernels import (
...
from flex_attn.kernels.common.backward_postprocess import FlexAttentionBackwardPostprocess
from flex_attn.kernels.common.backward_preprocess import FlexAttentionBackwardPreprocess
from flex_attn.kernels.sm90.backward import FlexAttentionBackwardSm90
from flex_attn.kernels.sm90.backward_config import resolve_sm90_bwd_consumer_config
from flex_attn.kernels.sm90.forward import FlexAttentionForwardSm90
from flex_attn.kernels.sm90.forward_config import (
    FwdConfig,
    _ResolvedSm90FwdConsumerConfig,
    resolve_sm90_fwd_consumer_config,
)
from flex_attn.kernels.sm100.bwd.backward import FlexAttentionBackwardSm100
from flex_attn.kernels.sm100.bwd.backward_config import resolve_sm100_bwd_consumer_config
...
from flex_attn.kernels.sm100.bwd.backward_hd256 import (
    BlackwellFusedMultiHeadAttentionBackward,
)
from flex_attn.kernels.sm100.fwd import (
...
from flex_attn.kernels.sm100.fwd.forward_hd256 import (
...
if os.environ.get("CUTE_DSL_PTXAS_PATH") is not None:
    from flex_attn.runtime import ptxas  # noqa: F401
...
def _get_device_arch() -> int:
    ...
    if arch not in SUPPORTED_ARCHES:
        raise NotImplementedError(
            f"flex_attn supports SM90, SM100, and SM103; got SM{arch}"
        )
    return arch
...
def _validate_head_dims(head_dim: int, head_dim_v: int, alignment: int) -> None:
    if not is_supported_head_dims(head_dim, head_dim_v):
        raise ValueError(
            f"supported head dimensions are {SUPPORTED_HEAD_DIM_RULE}; got "
...
def _validate_tensor(
    ...
    if tensor.shape != expected_shape:
        raise ValueError(
            f"{name} shape {tuple(tensor.shape)} != expected {tuple(expected_shape)}"
        )
    ...
    if tensor.device != expected_device:
        raise ValueError(
            f"{name} device {tensor.device} != expected {expected_device}"
        )
...
def _validate_mode_geometry(
    ...
    if (cu_seqlens_q is None) != (cu_seqlens_k is None):
        raise ValueError(
            "true-varlen attention requires both cu_seqlens_q and cu_seqlens_k"
        )
    if is_varlen and (max_seqlen_q is None or max_seqlen_k is None):
        raise ValueError(
            "true-varlen attention requires max_seqlen_q and max_seqlen_k"
        )
...
def _validate_plan_binding(
    ...
    *,
    ...
    expected_arch_family = (
        "sm90" if arch == 90 else canonical_blackwell_arch_family(arch)
    )
    if signature.arch_family != expected_arch_family:
        raise ValueError(
            f"{context} arch_family mismatch: expected "
            f"{expected_arch_family!r}, got {signature.arch_family!r}"
        )
    validate_arbitrary_plan_runtime_binding(
    ...
    *,
    ...
    if head_dim == 256 and head_dim_v == 256:
        ...
        return resolve_sm100_hd256_fwd_consumer_config(
            ...
            pack_gqa=pack_gqa,
            cta_group_size=(
                2 if kernel_family == "sm100_hd256_qstage1_2cta_fwd" else 1
            ),
        )
...
def _flex_attn_fwd(
    ...
    normalized_plan = normalize_arbitrary_block_sparse_config(
        ...
        expected_fixed_total_m_blocks=(
            None
            if is_varlen
            else batch_size * math.ceil(
                seqlen_q * qratio_plan / plan_tile_m
            )
        ),
    )
    ...
    qstage1_overlap_pv_with_k_wait = (
        use_qstage1_1cta and not normalized_plan.narrow_workset
    )

    output_shape = (
        (total_q, num_q_heads, head_dim_v)
        if is_varlen
        else (batch_size, seqlen_q, num_q_heads, head_dim_v)
    )
    lse_shape = (
        (num_q_heads, total_q)
        if is_varlen
        else (batch_size, num_q_heads, seqlen_q)
    )
    ...
    if compile_key not in _flex_attn_fwd.compile_cache:
        q_tensor, k_tensor, v_tensor, o_tensor = [
            to_cute_tensor(tensor) for tensor in (q, k, v, out)
        ]
        lse_tensor = (
        ...
        if arch == 90:
            ...
            kernel_cls = (
                ...
                else FlexAttentionForwardSm100
            )
            kernel_kwargs = (
...
def _make_fake_bwd_tensors(dtype, has_gqa: bool, is_varlen: bool):
    ...
    q = fake_tensor(
        dtype,
        (*q_shape, num_q_heads, head_dim),
        divisibility=divisibility,
    )
    k = fake_tensor(
        dtype,
        (*k_shape, num_kv_heads, head_dim),
        divisibility=divisibility,
    )
    v = fake_tensor(
        dtype,
        (*k_shape, num_kv_heads, head_dim_v),
        divisibility=divisibility,
    )
    out = fake_tensor(
        dtype,
        (*q_shape, num_q_heads, head_dim_v),
        divisibility=divisibility,
    )
    dout = fake_tensor(
        dtype,
    ...
    if is_varlen:
        lse = fake_tensor(Float32, (num_q_heads, total_q), divisibility=1)
        lse_log2 = fake_tensor(
            Float32, (num_q_heads, q_rounded), divisibility=4
        )
        dpsum = fake_tensor(
            Float32, (num_q_heads, q_rounded), divisibility=4
        )
        dq_accum = fake_tensor(
            Float32, (num_q_heads, q_d_rounded), divisibility=4
        )
        dk_accum = fake_tensor(
            Float32, (num_kv_heads, k_d_rounded), divisibility=4
        )
        dv_accum = fake_tensor(
            Float32, (num_kv_heads, k_dv_rounded), divisibility=4
        )
    else:
        lse = fake_tensor(
            Float32, (batch, num_q_heads, seqlen_q), divisibility=1
        )
        lse_log2 = fake_tensor(
            Float32, (batch, num_q_heads, q_rounded), divisibility=4
        )
        dpsum = fake_tensor(
            Float32, (batch, num_q_heads, q_rounded), divisibility=4
        )
        dq_accum = fake_tensor(
            Float32, (batch, num_q_heads, q_d_rounded), divisibility=4
        )
        dk_accum = fake_tensor(
            Float32, (batch, num_kv_heads, k_d_rounded), divisibility=4
        )
        dv_accum = fake_tensor(
            Float32, (batch, num_kv_heads, k_dv_rounded), divisibility=4
        )
    return (
        q,
        k,
...
def _compile_bwd_preprocess(
    ...
    cu_q = (
        fake_tensor(Int32, (cute.sym_int(),), divisibility=1)
        if is_varlen
        else None
    )
    dlse = fake_tensor(Float32, lse.shape, divisibility=1) if has_dlse else None
    ...
    *,
    ...
    if compile_key not in _bwd_postprocess_convert.compile_cache:
        _bwd_postprocess_convert.compile_cache[compile_key] = (
            _compile_bwd_postprocess(*compile_key)
        )
    if not is_fake_mode():
        _bwd_postprocess_convert.compile_cache[compile_key](
            accum,
...
def _resolve_bwd_compile_options(
    *,
    ...
    options = "--enable-tvm-ffi"
    if (arch == 103 and use_2cta) or (
        arch == 100 and use_2cta and use_hd256
    ):
        options += " --opt-level 2"
    return options
...
def _flex_attn_bwd(
    ...
    if nested_plan is None:
        raise ValueError(
            "arbitrary backward requires a MaskPlan built with backward metadata"
        )
    ...
    if (
        nested_plan.topology_tensors.runtime_binding
        is not block_sparse_tensors.topology_tensors.runtime_binding
    ):
        raise ValueError(
            "outer and nested backward plans must share the runtime binding"
        )

    use_hd256 = (
        arch in (100, 103) and head_dim == 256 and head_dim_v == 256
    )
    dq_plan = None
    dq_signature = None
    dq_config = None
    if use_hd256:
        if dlse is not None:
            raise NotImplementedError(
                "SM100 head-dim 256 backward does not support an LSE gradient"
            )
        if utils.get_disable_2cta_default():
            raise NotImplementedError(
                "SM100 head-dim 256 arbitrary backward requires 2CTA"
            )
        dq_plan = block_sparse_tensors.dq_tensors
        if dq_plan is None:
            raise ValueError(
                "SM100 head-dim 256 backward requires an independent dQ plan"
            )
        ...
        if use_2cta and utils.get_disable_2cta_default():
            raise NotImplementedError(
                "the resolved arbitrary backward plan requires 2CTA"
            )
    ...
    head_dim_v_rounded = (
        math.ceil(head_dim_v / accum_multiple) * accum_multiple
    )
    if is_varlen:
        total_q_padded = (
            (total_q + cu_seqlens_q.shape[0] * tile_m - 1)
            // tile_m
            * tile_m
        )
        dq_accum = (
    ...
    dkv_postprocess = not use_hd256 and (
        ...
        or (
            arch in (100, 103)
            and (
                head_dim_rounded != head_dim
                or head_dim_v_rounded != head_dim_v
            )
        )
    )
    ...
    normalized_bwd = normalize_arbitrary_block_sparse_config_bwd(
        ...
        expected_fixed_total_n_blocks=(
            None
            if is_varlen
            else batch_size * math.ceil(seqlen_k / bwd_config.block_size[1])
        ),
    ...
    if use_hd256:
        ...
        normalized_dq = normalize_arbitrary_block_sparse_config(
            ...
            expected_fixed_total_m_blocks=(
                None
                if is_varlen
                else batch_size * math.ceil(seqlen_q / dq_config.block_size[0])
            ),
        )
    elif deterministic:
        if normalized_bwd.dq_write_order is None:
            raise ValueError(
                "deterministic backward requires partial-block dQ write order"
            )
        if (
            normalized_bwd.full_block_cnt is not None
            and normalized_bwd.dq_write_order_full is None
        ):
            raise ValueError(
                "deterministic backward requires full-block dQ write order"
            )
        if normalized_bwd.spt is None:
            raise ValueError(
                "deterministic backward requires an SPT direction contract"
            )
    ...
    if compile_key not in _flex_attn_bwd.compile_cache:
        q_tensor, k_tensor, v_tensor, do_tensor, dq_tensor, dk_tensor, dv_tensor = [
            to_cute_tensor(tensor)
            for tensor in (q, k, v, dout, dq, dk, dv)
        ]
        lse_log2_tensor = to_cute_tensor(lse_log2)
        dpsum_tensor = to_cute_tensor(dpsum)
        dq_accum_tensor = (
            dq_tensor if use_hd256 else to_cute_tensor(dq_accum)
        )
        dk_output_tensor = (
            to_cute_tensor(dk_accum) if dkv_postprocess else dk_tensor
        )
        dv_output_tensor = (
            to_cute_tensor(dv_accum) if dkv_postprocess else dv_tensor
        )
        cu_q_tensor, cu_k_tensor = [
            to_cute_tensor(tensor, assumed_align=4)
            if tensor is not None
            else None
            for tensor in (cu_seqlens_q, cu_seqlens_k)
        ]
        ...
        sparse_dq_tensor = (
            to_cute_block_sparse_tensors(normalized_dq)
            if normalized_dq is not None
            else None
        )
    ...
    if not use_hd256:
        num_threads_post_q = (
            128 if arch == 90 and dq_single_wg else
            bwd_config.num_wg * 128 if arch == 90 else
            128
        )
        num_threads_post_kv = (
            bwd_config.num_wg * 128 if arch == 90 else 128
        )
