...
# Enable cache via `FLEX_ATTENTION_CUTE_DSL_CACHE_ENABLED=1`
CUTE_DSL_CACHE_ENABLED: bool = os.getenv("FLEX_ATTENTION_CUTE_DSL_CACHE_ENABLED", "0") == "1"


# Customize cache dir via `FLEX_ATTENTION_CUTE_DSL_CACHE_DIR`, default is
# `/tmp/${USER}/flex_attention_cute_dsl_cache``
CUTE_DSL_CACHE_DIR: str | None = os.getenv("FLEX_ATTENTION_CUTE_DSL_CACHE_DIR", None)
...
def get_cache_path() -> Path:
    if CUTE_DSL_CACHE_DIR is not None:
        ...
        cache_dir = Path(tempfile.gettempdir()) / getuser() / "flex_attention_cute_dsl_cache"
...
def _compute_source_fingerprint() -> str:
    ...
    - Any Python source under flex_attn is added, removed, renamed, or modified.
    ...
    """
    cute_root = Path(__file__).resolve().parents[1]
    h = hashlib.sha256()
